...
.edge-label.radio { color: #0a5c3b; background: #d4f5e5; border-color: #9fdcc0; }
...
<div class="container">
    ...
    (function() {
        ...
        function drawLine(svg, canvas, fromEl, toEl, iface, cssClass, offset) {
            ...
            var color = {'sbi':'#9333ea','n2n3':'#185FA5','xn':'#0a5c3b','radio':'#1D9E75'}[cssClass] || '#94a3b8';
    ...
    <!-- Infrastructure Status (แนวนอน ใต้ topology) -->
    <div class="section" style="margin-bottom:1.25rem;">
        <div class="section-header">
            Infrastructure Status
            <span class="ns-badge ns-default">kube-system</span>
        </div>
        <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:1px; background:#ebebeb;">
            <!-- Flannel -->
            <div style="background:white; padding:0.9rem 1.1rem;">
                <div style="font-size:11px; font-weight:700; color:#555; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:6px;">Flannel CNI</div>
                {% if infra.flannel_ok %}
                <span class="topo-status Running" style="font-size:12px;">✅ OK</span>
                <div style="font-size:11px; color:#888; margin-top:5px; font-family:monospace;">subnet: {{ infra.flannel_subnet }}</div>
                {% else %}
                <span class="topo-status NotRunning" style="font-size:12px;">❌ subnet.env หาย</span>
                {% endif %}
            </div>
            <!-- CoreDNS -->
            <div style="background:white; padding:0.9rem 1.1rem;">
                <div style="font-size:11px; font-weight:700; color:#555; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:6px;">CoreDNS</div>
                {% if infra.coredns_running > 0 %}
                <span class="topo-status Running" style="font-size:12px;">✅ {{ infra.coredns_running }}/{{ infra.coredns_total }} Running</span>
                {% else %}
                <span class="topo-status NotRunning" style="font-size:12px;">❌ Not Running</span>
                {% endif %}
            </div>
            <!-- NRF ClusterIP -->
            <div style="background:white; padding:0.9rem 1.1rem;">
                <div style="font-size:11px; font-weight:700; color:#555; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:6px;">NRF ClusterIP</div>
                {% if infra.nrf_ip %}
                <span class="topo-status Running" style="font-size:12px;">✅ {{ infra.nrf_ip }}</span>
                {% else %}
                <span class="topo-status NotRunning" style="font-size:12px;">❌ ไม่พบ</span>
                {% endif %}
            </div>
        </div>
    </div>
...
def get_infra_status() -> dict:
    """เช็ค Flannel subnet.env และ CoreDNS"""
    import os
    # Flannel
    flannel_ok = os.path.exists('/run/flannel/subnet.env')
    flannel_subnet = ""
    if flannel_ok:
        try:
            with open('/run/flannel/subnet.env') as f:
                for line in f:
                    if line.startswith('FLANNEL_SUBNET'):
                        flannel_subnet = line.strip().split('=')[1]
        except:
            pass
